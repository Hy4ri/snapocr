...
impl SnapApp {
    fn begin_ocr(&mut self, sel: Rect) {
        ...
        self.stage = Stage::Ocr(Some(std::thread::spawn(move || {
            ...
            // Upscaling 2.5x with CatmullRom turns small blurry UI glyphs into crisp characters.
            ...
            // If lang is auto or empty, resolve all available languages dynamically (e.g. eng+ara)
...
/// Discovers installed Tesseract language models and joins them for auto-detection (e.g. "eng+ara")
fn detect_available_langs() -> Option<String> {
    ...
    for line in stdout.lines() {
        let line = line.trim();
        // Skip header lines or helper models like osd
        if line.is_empty()
    ...
    if langs.is_empty() {
        ...
        // Preferred order: english first, then others
        langs.sort_by(|a, b| {
            if *a == "eng" {
